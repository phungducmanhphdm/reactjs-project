...
function AddSpecifyForPage() {
  const handleValid = useCallback((e) => {
    const value = e.target.value;
    if (!value) {
      return {
        status: false,
        message: "Specify name cannot be empty.",
      };
    }
  ...
  useEffect(() => {
    SpecifyFor.checkName(
      debounce,
      (res) => {
        if (res.data.length > 0) {
          data.valid = {
            status: false,
            message: "The specify name already exists.",
          };
...
export default AddSpecifyForPage;
